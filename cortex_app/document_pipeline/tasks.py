...
def on_process_document_failure(self, exc, task_id, args, kwargs, einfo):
...
def on_ingest_s3_failure(self, exc, task_id, args, kwargs, einfo):
    doc_id = args[0] if args else kwargs.get("doc_id")
...
async def _run_device_pipeline(doc_id: str) -> None:
    ...
    async with get_custom_db_context_session() as db:
        ...
        storage_key = f"kb/{kb_id}/{doc_id}/{filename}"

        # --- 1. uploading ---
        doc.processing_status = KbProcessingStatusEnum.uploading
        await db.commit()
        await _publish_status(redis_client, user_id, doc_id, "uploading", filename)
        ...
        await db.commit()
        await _publish_status(redis_client, user_id, doc_id, "processing", filename)
...
async def _run_s3_pipeline(doc_id: str, s3_url: str, creds: dict) -> None:
    ...
    async with get_custom_db_context_session() as db:
        ...
        from document_pipeline.storage import multipart_upload_bytes
...
async def _ingest_from_storage(db, doc, kb_id: str, doc_id: str, filename: str,
    ...
    from app.knowledge_bases.db_models import KbProcessingStatusEnum
    ...
    raw_chunks = parse_document(file_bytes, filename, openai_api_key)
    if not raw_chunks:
    ...
    await db.commit()
    await _publish_status(redis_client, user_id, doc_id, "ready", filename, chunk_count=len(chunks))
        ...
        async with get_custom_db_context_session() as db:
            ...
            if doc:
                ...
                await _publish_status(
                    redis_client, str(doc.user_id), doc_id, "failed", doc.filename,
...
async def _publish_status(
    ...
    import json
    payload = {"doc_id": doc_id, "status": status, "filename": filename}
    ...
    try:
        ...
        logger.warning("Failed to publish KB status: %s", payload)
